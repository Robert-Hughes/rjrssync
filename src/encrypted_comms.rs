    ...
    where T : Serialize,
{
    let unencrypted_data = {
        ...
        bincode::serialize(&x).map_err(|e| "Error serializing command: ".to_string() + &e.to_string())?
    };
    ...
    let ciphertext = {
        ...
        cipher.encrypt(nonce, unencrypted_data.as_ref()).unwrap()
    };

    {
        profile_this!("Tcp_Write");
