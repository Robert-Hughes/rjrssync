...
/// Syncing three files which already exists on the dest, with the same modified dates.
...
fn prompt_skip_then_overwrite() {
    ...
    run(TestDesc {
        ...
        args: vec![
            "$TEMP/src".to_string(),
            "$TEMP/dest".to_string(),
            "--files-same-time=prompt".to_string(),
        ],
        ...
        expected_output_messages: vec![
            (1, Regex::new("dest file .*c1' has the same modified time as source file .*c1.*").unwrap()),
            (1, Regex::new("dest file .*c2' has the same modified time as source file .*c2.*").unwrap()),
            ...
            (1, Regex::new("dest file .*c3' has the same modified time as source file .*c3.*").unwrap()),
...
/// Syncing two files which already exists on the dest, with the same modified dates.
/// The expected behaviour is controlled by a command-line argument, which in this case
...
fn prompt_skip_all() {
    ...
    run(TestDesc {
        ...
        expected_exit_code: 0,
        expected_output_messages: vec![
            (1, Regex::new("dest file .*c\\d' has the same modified time as source file .*c\\d.*").unwrap()), // We can't be sure which file we'll be prompted about, because the order is non-deterministic, we just need to make sure there's only one
...
/// Syncing a file which already exists on the dest, with the same modified date.
/// The expected behaviour is controlled by a command-line argument, which in this case
...
fn prompt_cancel() {
    ...
    run(TestDesc {
        ...
        expected_output_messages: vec![
            ...
            (2, Regex::new("dest file .*c1' has the same modified time as source file .*c1.*").unwrap()),
            (1, Regex::new(&regex::escape("Errorring as requested")).unwrap()), // Cancelled
        ],
        expected_filesystem_nodes: vec![
            ("$TEMP/src", Some(&src)), // Unchanged
            ("$TEMP/dest", Some(&dest)), // Unchanged
...
fn error() {
    ...
    run(TestDesc {
        ...
        args: vec![
            "$TEMP/src".to_string(),
            "$TEMP/dest".to_string(),
            "--files-same-time=error".to_string(),
        ],
        expected_exit_code: 12,
        expected_output_messages: vec![
            (1, Regex::new(&regex::escape("Errorring as requested")).unwrap()),
        ],
        expected_filesystem_nodes: vec![
            ("$TEMP/src", Some(&src)), // Unchanged
            ("$TEMP/dest", Some(&dest)), // Unchanged
...
fn skip() {
    ...
    run(TestDesc {
        ...
        args: vec![
            "$TEMP/src".to_string(),
            "$TEMP/dest".to_string(),
            "--files-same-time=skip".to_string(),
        ],
        expected_exit_code: 0,
        expected_output_messages: vec![
...
fn overwrite() {
    ...
    run(TestDesc {
        ...
        args: vec![
            "$TEMP/src".to_string(),
            "$TEMP/dest".to_string(),
            "--files-same-time=overwrite".to_string(),
        ],
        expected_exit_code: 0,
        expected_output_messages: vec![
